...
class EnsembleDataset(Dataset):
    ...
    def get_sample_tensor(self, idx: int) -> Tensor:
        #context = self.tensor_data[idx:idx + self.modulus*self.context_length:self.modulus].T.detach().clone()
        context = self.tensor_data[idx:idx + self.modulus*self.context_length].T.detach().clone()
        return context

    def __len__(self) -> int:
        return max(0, self.tensor_data.shape[0] - self.modulus*self.context_length - 1 - self.prediction_offset)

    def __getitem__(self, idx: int) -> Tensor:
        context = self.tensor_data[idx:idx + self.modulus*self.context_length:self.modulus].T.detach().clone()
        target = self.tensor_data[idx + self.modulus*self.context_length + self.prediction_offset, self.ts_columns].detach().clone()
